...
function getPizzaOrder() {
  ...
  numOfToppings = toppings.length;
  if(toppings == ""){
    numOfToppings = 0;
  }
  
  for(let x=0; x<numOfToppings; x++){
    pizza.toppings.push(toppings[x].trim());
  }

  costOfToppings = numOfToppings * toppingsFee;

  pizza.cost += costOfToppings;
